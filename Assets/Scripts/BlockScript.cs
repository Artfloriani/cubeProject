...
public class BlockScript : MonoBehaviour {
	...
	void Start()
	{
		...
		seed = Random.RandomRange(10.0f, 20.0f);
			...
			if (_distFrPlayer > blockSize * 4.0f) {
				if (displayingCubes) {
					...
					foreach (KeyValuePair<Vector3, GameObject> entry in cubePositions) {
						...
						entry.Value.GetComponent<BlockScript> ().boudingCube.SetActive (false);
					}
				...
				if (!displayingCubes) {
					...
					foreach (KeyValuePair<Vector3, GameObject> entry in cubePositions) {
						...
						if (entry.Value.GetComponent<BlockScript> ().boudingCube != null)
							entry.Value.GetComponent<BlockScript> ().boudingCube.SetActive (true);
					}
	...
	void addCube(Vector3 cubePosition)
	{
		...
		else
		{
			...
			cube.GetComponent<BlockScript>().cubeSize = this.cubeSize/10f;
			cube.GetComponent<BlockScript>().blockSize = this.cubeSize;
			//cube.transform.SetParent(this.transform);
